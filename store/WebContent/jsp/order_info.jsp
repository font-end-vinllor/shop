						...
						<tbody>
							<tr class="warning">
								...
								<c:if test="${order.state == 3 }">已完成</c:if>
								 ...
								 <th colspan="2">下单时间:<fmt:formatDate value="${order.ordertime }" pattern="YYYY-MM-dd HH:mm:ss"/>
								 </th>
							</tr>
							<tr class="warning">
								<th>图片</th>
							...
							<c:forEach items="${order.orderItem }" var="it">
							<tr class="active">
								<td width="60" width="40%">
									<input type="hidden" name="id" value="22">
									<img src="${pageContext.request.contextPath}/${it.product.pimage}" width="70" height="60">
								</td>
								<td width="30%">
									<a target="_blank"> ${it.product.pname} }</a>
								</td>
								<td width="20%">
									￥${it.product.shop_price }
								</td>
								<td width="10%">
									${it.count }
								</td>
